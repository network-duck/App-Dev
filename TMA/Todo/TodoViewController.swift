...
class TodoViewController: UITableViewController {
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if let eventsForDate = allEvents[date] {
            let sortedEvents = eventsForDate.sorted(by: { $0.name < $1.name })
    ...
    override func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        if let eventsForDate = allEvents[date], indexPath.row < eventsForDate.count {
            ...
            performSegue(withIdentifier: "editEventSegue", sender: selectedEvent)
        } else {
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        if segue.identifier == "editEventSegue", let eventEditVC = segue.destination as? EventEditViewController,
